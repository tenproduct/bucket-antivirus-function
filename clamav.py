...
def update_defs_from_s3(bucket, prefix):
    common.create_dir(common.AV_DEFINITION_PATH)
    for filename in common.AV_DEFINITION_FILENAMES:
        s3_path = os.path.join(common.AV_DEFINITION_S3_PREFIX, filename)
        local_path = os.path.join(common.AV_DEFINITION_PATH, filename)
        ...
        if s3_md5:
            ...
            common.s3.Bucket(bucket).download_file(s3_path, local_path)
    ...
    for filename in common.AV_DEFINITION_FILENAMES:
        ...
        if os.path.exists(local_file_path):
            ...
            if local_file_md5 != md5_from_s3_tags(bucket, os.path.join(prefix, filename)):
                ...
                s3_object = common.s3.Object(bucket, os.path.join(prefix, filename))
...
def update_defs_from_freshclam(path, library_path=''):
    common.create_dir(path)
    fc_env = os.environ.copy()
    if library_path:
        fc_env['LD_LIBRARY_PATH'] = '%s:%s' % (
            ':'.join(current_library_search_path()), common.CLAMAVLIB_PATH,
        )
    print('Starting freshclam with defs in %s.' % path)
    fc_proc = Popen(
        [
            common.FRESHCLAM_PATH,
...
def md5_from_s3_tags(bucket, key):
    try:
        tags = common.s3_client.get_object_tagging(Bucket=bucket, Key=key)['TagSet']
...
def scan_file(path):
    ...
    av_env['LD_LIBRARY_PATH'] = common.CLAMAVLIB_PATH
    print('Starting clamscan of %s.' % path)
    av_proc = Popen(
        [
            common.CLAMSCAN_PATH,
            '-v',
            '-a',
            '--stdout',
            '-d',
    ...
    if av_proc.returncode == 0:
        return common.AV_STATUS_CLEAN
    elif av_proc.returncode == 1:
        return common.AV_STATUS_INFECTED
    else:
