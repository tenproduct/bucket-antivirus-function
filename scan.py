...
def event_object(event):
    ...
    return common.s3.Object(bucket, key)
...
def verify_s3_object_version(s3_object):
    ...
    # downstream services may consume latest version by mistake and
    # get the infected version instead
    if str_to_bool(common.AV_PROCESS_ORIGINAL_VERSION_ONLY):
        bucketVersioning = common.s3.BucketVersioning(s3_object.bucket_name)
        if (bucketVersioning.status == 'Enabled'):
            bucket = common.s3.Bucket(s3_object.bucket_name)
            ...
            print(
                'Unable to implement check for original version, '
                'as versioning is not enabled in bucket %s' % s3_object.bucket_name
            )
            raise Exception(
...
def download_s3_object(s3_object, local_prefix):
    ...
    common.create_dir(os.path.dirname(local_path))
...
def set_av_metadata(s3_object, result):
    ...
    metadata[common.AV_STATUS_METADATA] = result
    metadata[common.AV_TIMESTAMP_METADATA] = datetime.utcnow().strftime('%Y/%m/%d %H:%M:%S UTC')
    s3_object.copy(
        {
...
def set_av_tags(s3_object, result):
    curr_tags = common.s3_client.get_object_tagging(
        Bucket=s3_object.bucket_name,
        Key=s3_object.key,
    )['TagSet']
    new_tags = copy.copy(curr_tags)
    for tag in curr_tags:
        if tag['Key'] in [common.AV_STATUS_METADATA, common.AV_TIMESTAMP_METADATA]:
            new_tags.remove(tag)
    new_tags.append({'Key': common.AV_STATUS_METADATA, 'Value': result})
    new_tags.append({
        'Key': common.AV_TIMESTAMP_METADATA,
        'Value': datetime.utcnow().strftime('%Y/%m/%d %H:%M:%S UTC'),
    })
    common.s3_client.put_object_tagging(
        Bucket=s3_object.bucket_name,
        Key=s3_object.key,
...
def sns_start_scan(s3_object):
    if common.AV_SCAN_START_SNS_ARN is None:
        return
    message = {
        'bucket': s3_object.bucket_name,
        'key': s3_object.key,
        'version': s3_object.version_id,
        common.AV_SCAN_START_METADATA: True,
        common.AV_TIMESTAMP_METADATA: datetime.utcnow().strftime('%Y/%m/%d %H:%M:%S UTC')
    }
    sns_client = boto3.client('sns')
    sns_client.publish(
        TargetArn=common.AV_SCAN_START_SNS_ARN,
...
def sns_scan_results(s3_object, result):
    ...
    message = {
        ...
        'key': s3_object.key,
        'version': s3_object.version_id,
        common.AV_STATUS_METADATA: result,
        common.AV_TIMESTAMP_METADATA: datetime.utcnow().strftime('%Y/%m/%d %H:%M:%S UTC')
    }
    sns_client = boto3.client('sns')
    sns_client.publish(
        TargetArn=common.AV_STATUS_SNS_ARN,
        Message=json.dumps({'default': json.dumps(message)}),
        MessageStructure='json'
    )
